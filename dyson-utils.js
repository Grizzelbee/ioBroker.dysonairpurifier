...
module.exports.getDyson2faMail = async function (
  ...
  try {
    const result = await axios.post(
      dysonConstants.API_BASE_URI +
        `/v3/userregistration/email/userstatus?country=${country}`,
      payload,
    ...
    if (result.data && result.data.accountStatus !== 'ACTIVE') {
      ...
      if (result.data.authenticationMethod === 'EMAIL_PWD_2FA') {
        const response = await axios.post(
          dysonConstants.API_BASE_URI +
            `/v3/userregistration/email/auth?country=${country}&culture=${locale}`,
          payload,
          { httpsAgent, headers: dysonConstants.HTTP_HEADERS, json: true }
        );
        adapter.log.debug(
...
module.exports.getDysonToken = async function (
  ...
  try {
    const response = await axios.post(
      dysonConstants.API_BASE_URI +
        `/v3/userregistration/email/verify?country=${country}`,
      payload,
      { httpsAgent, headers: dysonConstants.HTTP_HEADERS, json: true }
    );
    // return(response.data);
 ...
 */ module.exports.getAngles = function (
  adapter,
  dysonAction,
  thisDevice,
  state
  ...
  return new Promise(async function (resolve) {
    ...
    if (dysonAction === 'ancp') {
      result.ancp = state;
      result.osal = await adapter.getStateAsync(
        thisDevice + '.OscillationLeft'
      );
      result.osau = await adapter.getStateAsync(
        thisDevice + '.OscillationRight'
      );
    } else if (dysonAction === 'osal') {
      result.ancp = await adapter.getStateAsync(
        thisDevice + '.OscillationAngle'
      );
      result.osal = state;
      result.osau = await adapter.getStateAsync(
        thisDevice + '.OscillationRight'
    ...
    this.dysonGetDevicesFromApi(token)
      .then(response => {
        ...
        for (const thisDevice in response.data) {
          adapter.log.debug(
            'Data received from dyson API: ' +
              JSON.stringify(response.data[thisDevice])
          );
          if (
            ...
            if (552 === response.data[thisDevice].ProductType) {
              adapter.log.info(
                'Device with serial number [' +
                  response.data[thisDevice].Serial +
                  '] not added, since it is a lamp and not supported by this adapter.'
              );
            } else {
              adapter.log.warn(
                'Device with serial number [' +
                  response.data[thisDevice].Serial +
                  '] not added, hence it is not supported by this adapter. Product type: [' +
                  response.data[thisDevice].ProductType +
                  ']'
              );
              adapter.log.warn(
